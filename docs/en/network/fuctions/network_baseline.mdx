...
Alauda Container Security (ACS) helps you minimize network risks by using network baselining. This proactive approach secures your infrastructure by learning normal network flows and identifying any deviations as anomalies.

## How Network Baselining Works

When you first install ACS, there is no default network baseline. As ACS observes network activity, it automatically adds discovered network flows to the baseline:
...
- ACS stops adding new flows to the baseline.
...
- To mark a single flow as anomalous, select the entity, click the overflow menu (![kebab](https://access.redhat.com/webassets/avalon/d/Red_Hat_Advanced_Cluster_Security_for_Kubernetes-4.7-Operating-en-US/images/f468284ec3cc9bf27e6bd2c83849ca50/kebab.png)), and choose **Mark as anomalous**.
...
You can adjust how long ACS observes network flows before finalizing the baseline using environment variables.
...
ACS can be configured to trigger violations for anomalous network flows (flows not in the baseline).
...
**Note:**  
- All references to "Red Hat Advanced Cluster Security" have been replaced with "Alauda Container Security".
- All references to "RHACS" have been replaced with "ACS".
- All references to "OpenShift Container Platform" have been replaced with "Alauda Container Platform".
- No Legal Notice section was present in the original document.
- Table formatting and MDX syntax have been reviewed and corrected.
- Document structure and headings have been optimized for clarity and readability.