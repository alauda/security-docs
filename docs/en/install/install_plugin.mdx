...
1. Download the Alauda Security Service for StackRox from the portal.

2. Use the Violet tool to upload the plugin package to the platform.
...
4. On the **Install Operator** page, select **Default Configuration**, then click **Install** to complete the installation of **Alauda Security Service for StackRox**.

## Central Service Installation

:::info
Some components of the Central service are resource-intensive. We recommend running them on infra nodes and setting nodeSelector and tolerations to ensure they run only on those nodes. If you are evaluating the product and have not provisioned infra nodes, you can remove these settings so the components run on all nodes.
...
| central.exposure.nodePort.port                         | The port number on which to expose Central. If you leave this parameter unset, the platform automatically assigns a port number. |
| central.exposure.ingress.enabled                       | Use true to expose Central by using an ingress. Disables all ingress settings if set to false. |
| central.exposure.ingress.host                          | Use this parameter to specify a custom hostname for the Central ingress. Leave this unset to accept the platform-provided default value. |
...
| Parameter                                             | Description |
| ----------------------------------------------------- | ----------- |
| clusterName | The unique name of this cluster, as it will be shown in the Central UI. Once a name is set here, you cannot change it later. To register the cluster with a different name, delete and recreate this object. |
| centralEndpoint | The endpoint of the Central instance to connect to, including the port number. If no port is specified and the endpoint contains an `https://` protocol specification, then port `443` is assumed. If you are using a non-gRPC-capable load balancer, use the WebSocket protocol by prefixing the endpoint address with `wss://`. When left blank, Sensor attempts to connect to a Central instance running in the same namespace. |
