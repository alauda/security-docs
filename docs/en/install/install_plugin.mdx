...
4. Create additional CA secret (if needed):
...
5. Create an ingressClass named `stackrox` as required by your environment.
...
|Ingress Host|Specify a custom hostname for the central ingress. Specify a "central-ingress-tls" ts type secret in stackrox namespace that contains tls.crt, tls.key.|
...
1. In Platform Configuration, go to **Clusters**.

2. Click the **Init bundles** button next to Central Service for StackRox.  
...
3. **Configure Ingress domain certificate (if needed):**
