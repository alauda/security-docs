---
title: 使用 roxctl 检查策略合规性
description: 了解如何在 Alauda 容器安全中使用 roxctl 检查策略合规性。
weight: 10
sourceSHA: 5deeb06f31fbec2e0697f32f15490b01cd3834ea6f5134abd0092d3a5fdc440f
---

# 使用 roxctl 检查策略合规性

Alauda 容器安全提供了 `roxctl` CLI，帮助您检查部署 YAML 文件和容器镜像的策略合规性。本指南将解释如何使用 `roxctl` 进行这些检查并解读结果。
...
> **注意：** 无论 `--verbose` 标志如何，额外的部署信息都包含在 JSON 输出中。
...
有关更多详细信息，请参阅 Alauda 容器安全文档和 API 参考。
