...
| 2   | `RHACS`     | `Platform Configuration > Clusters > Cluster registration secrets`                                | 集群注册密钥页面，用于创建和管理新集群接入 Central 所需的初始信任凭据。                | `please consult the RHACS documentation for details`                                                                           | 可改  | 这是帮助文案里的外部文档引用，不应直接指向上游产品名。                 | 隐藏该按钮/入口；如果后续仍保留能力，再补 Alauda Security Service 文档入口。                              | 确认隐藏此按钮                   |
